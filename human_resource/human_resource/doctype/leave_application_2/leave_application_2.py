...
class LeaveApplication2(Document):
    ...
    def check_leave_balance(self):
        negative_balance = frappe.db.sql(""" select negative_balance from `tabLeave Type2` 
    ...
    def update_balance_allocation_after_submit(self):
        ...
        and to_date >= %s""", (new_balance_allocation, self.employee, self.leave_type, self.from_date, self.to_date),
                      as_dict=1)
    ...
    def check_for_duplication(self):
        leave_file = frappe.db.sql(""" select * from `tabLeave Application 2` where from_date between %s and %s 
        and to_date between %s and %s and employee = %s and leave_type = %s """, (self.from_date, self.to_date, self.from_date, self.to_date, self.employee, self.leave_type),
                                   as_dict=1)
        if leave_file:
            frappe.throw("This leave application already exists!")

    def check_max_days(self):
        max_days = frappe.db.sql(""" select max_days, negative_balance from `tabLeave Type2` 
        where leave_type_name = %s""", (self.leave_type))
    ...
    def validate_applicable_after(self):
        applicable_after = frappe.db.sql(""" select applicable_after from `tabLeave Type2` 
        where leave_type_name = %s""", (self.leave_type))
